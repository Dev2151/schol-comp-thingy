...
This app now supports **distributed AI inference** — running large language models across multiple machines on the same network. A coordinator PC orchestrates the cluster while worker PCs (or a VM standing in for one) process their assigned portions of the model, combining RAM to run models that wouldn't fit on a single machine. The inference engine is llama.cpp's RPC backend (MIT licensed); Ollama is used only as a model warehouse.
...
The architecture is best understood as two independent networks overlaid on the same machines.
...
## The lifecycle of one question, step by step

`./setup ask "What is 7 times 8?"`

1. **Bash** sets `LD_LIBRARY_PATH` and execs `llama-cli -m qwen25-3b.gguf --rpc 127.0.0.1:19556 -p ... -st`.
2. **RPC handshake.** llama-cli connects through the SSH tunnel to the guest's rpc-server; they exchange protocol version and device descriptions ("CPU, 3910 MiB free").
...
**Why the terminal node replaced the Electron worker?** A node's job is to be present, honest about its resources, and diagnosable — none of which needs a GUI toolkit, a Chromium instance, or 300MB of node_modules. The terminal dashboard renders at 1 Hz with ANSI escapes; the control port answers plain text. It boots faster, uses ~20MB, and can be driven from the host with a five-line Python snippet — no agent needed.

## Honest limits (the Q&A page of this document)

- **Throughput** is 2.7 t/s: the bottleneck is tensor serialization over a tunnel, not compute. Phase-2 fixes are known upstream patterns (activation compression a la Petals, direct LAN RPC without the tunnel, multiple remotes).
...
Everything is driven from the repo root `./setup` script:

| Command | What it does |
|---|---|
| `./setup start` | Bring up the full cluster (VM, node, RPC server, tunnel, coordinator) |
| `./setup stop` | Tear everything down |
| `./setup ask "question"` | Status line, then prompt and answer in plain text |
| `./setup ask -q "question"` | Answer only (quiet mode, good for scripts) |
| `./setup status` | Cluster health check |
| `./setup demo` | 30-second scripted demo reel |
| `./setup help` | Command dictionary |

Example, verified end to end through the VM's layers:
