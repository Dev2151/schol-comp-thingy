# Title TBD — Full Architecture

## 1. The master flow chart

```
┌─────────────────────────────────────────────────────────────────────────────────────────┐
│                                    HOST LAPTOP (ty-20nks0qn15)                          │
│                                                                                         │
│  ┌────────────────────────────────────┐        ┌─────────────────────────────────────┐  │
│  │   ELECTRON COORDINATOR             │        │   LLAMA.CPP TENSOR ENGINE           │  │
│  │   (node_modules/electron, port     │        │   (~/llamacpp/llama-b11191)         │  │
│  │    5173 UI via vite, 9501 TCP)     │        │                                     │  │
│  │                                    │        │  llama-server                       │  │
│  │  main.ts ── spawns ──> tcp-server  │        │   ├─ loads qwen25-3b.gguf           │  │
│  │    .ts                             │        │   ├─ --rpc 127.0.0.1:19556          │  │
│  │                                    │        │   │    registers VM as 2nd backend  │  │
│  │  ipc-handlers.ts (257 lines)       │        │   └─ ggml_backend_sched:            │  │
│  │  preload.ts (safe API bridge)      │        │        layer 0..7   → local CPU     │  │
│  │                                    │        │        layer 8..25  → RPC backend   │  │
│  │  shared/model-selection.ts         │        │        (offload = whole model when  │  │
│  │   └─ reads MemAvailable from       │        │         remote RAM allows)          │  │
│  │      every heartbeat               │        │                                     │  │
│  │   └─ picks biggest model that      │        │  llama-cli  ←── you type here       │  │
│  │      fits EVERY node               │        │   -m qwen25-3b.gguf                 │  │
│  │   └─ issues ASSIGN_LAYERS          │        │   --rpc 127.0.0.1:19556             │  │
│  │                                    │        │   -p "..." -n N --temp 0 -st        │  │
│  │  renderer/src/pages/* (Chat,       │        └──────────────┬──────────────────────┘  │
│  │  Cluster, Dashboard, Pipeline,     │                       │                         │
│  │  Models, Settings)                 │                       │                         │
│  └──────────┬─────────────────────────┘                       │                         │
│             │ TCP 9501 (binds 0.0.0.0)                        │                         │
│             │                                     ssh -L 19556:127.0.0.1:9555           │
│             │                                     (SSH tunnel = tensor highway)         │
│             │                                               │                           │
│  ┌──────────┴────────────┐                                  │                           │
│  │   OLLAMA (port 11434) │◄── auto ollama pull              │                           │
│  │   Model WAREHOUSE:    │    when picker selects           │                           │
│  │   stores GGUF blobs,  │    a model a node lacks          │                           │
│  │   serves whole-       │                                  │                           │
│  │   completions only.   │                                  │                           │
│  │   DOES NOT SPLIT.     │                                  │                           │
│  └───────────────────────┘                                  │                           │
│                                                             │                           │
│  scripts/start-cluster.sh  ── orchestrates ALL of the above + VM      │                 │
│  ./setup                    ── thin dispatcher to start-cluster.sh    │                 │
│  scripts/vm-run.sh          ── QEMU launch line                       │                 │
└─────────────────────────────────────────────────────────────┼─────────┼─────────────────┘
                                                              │         │
                              QEMU user-mode networking (slirp NAT)      │
                              hostfwd: 2222→22(ssh), 9555→9555(rpc),     │
                                       9666→9666(control)                │
                                                              │         │
┌─────────────────────────────────────────────────────────────┼─────────┼─────────────────┐
│                LUBUNTU VM "lubuntu-vm"  (4GB RAM, 4 vCPU)   │         │                 │
│                                                             ▼         ▼                 │
│  ┌──────────────────────────────────┐    ┌──────────────────────────────────────────┐   │
│  │  TTD-NODE.PY  (terminal node)    │    │  GGML-RPC-SERVER  (llama.cpp, 195KB)     │   │
│  │                                  │    │                                          │   │
│  │  connection_loop():              │    │  --host 0.0.0.0 --port 9555              │  │
│  │   ├─ WORKER_HELLO ───────────────┼────┼─► (separate channel from tensors)        │   │
│  │   │   {nodeId, hostname,         │    │                                          │   │
│  │   │    freeRam, cpuModel}        │    │  Receives serialized input tensors       │   │
│  │   ├─ ASSIGN_LAYERS ◄─┐           │    │  from host llama-server                  │   │
│  │   │   displays "gemma2:2b   │      │    │  Runs matmuls for its assigned subgraph  │   │
│  │   │   L8-25 (18 layers)"        │    │  on 4 vCPUs                              │   │
│  │   ├─ HEARTBEAT_ACK ◄─┘           │    │  Ships output hidden states back         │   │
│  │   │   (freeRam in every ack)     │    │                                          │   │
│  │   └─ reconnect: backoff 3s→30s   │    │  ALSO answers hostfwd 9555 directly      │   │
│  │       forever, last known coord  │    │  (that path was flaky through slirp,     │   │
│  │                                  │    │   hence the SSH tunnel)                  │   │
│  │  ctrl_server() port 9666:        │    └──────────────────────────────────────────┘   │
│  │   STATUS / LOGS / CONNECT /      │                                                  │
│  │   LAYERS / PING / QUIT           │    ┌──────────────────────────────────────────┐   │
│  │   ← manipulated from host        │    │  sshd (systemctl, port 22)               │   │
│  │   through hostfwd 9666           │    │  authorized_keys = host ed25519 key      │   │
│  │                                  │    │  ← how host runs commands in guest       │   │
│  │  render(): 1Hz ANSI dashboard    │    └──────────────────────────────────────────┘   │
│  │   status, layers, RAM bar,       │                                                  │
│  │   event log                      │    ┌──────────────────────────────────────────┐   │
│  │                                  │    │  OLLAMA (guest, port 11434)              │   │
│  │  ~/.config/autostart/:           │    │  same warehouse role: pulled gemma2:2b,  │   │
│  │   ttd-node.desktop               │    │  serves whole completions if asked,      │   │
│  │   ttd-rpc.desktop                │    │  NOT part of the split path              │   │
│  │   ← self-assembly on boot        │    └──────────────────────────────────────────┘   │
│  └──────────────────────────────────┘                                                  │
└─────────────────────────────────────────────────────────────────────────────────────────┘
```
